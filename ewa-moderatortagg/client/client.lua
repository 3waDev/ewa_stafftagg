...
AddEventHandler('esx:playerLoaded', function()
    ESX.TriggerServerCallback('ewa-moderatorTag:server:requestSync', function(request)
        stafftable = request
    end)
end)

RegisterNetEvent('ewa-moderatorTagg:client:syncTags')
AddEventHandler('ewa-moderatorTagg:client:syncTags', function(sync)
    stafftable = sync
end)

RegisterCommand(Ewa.commandName, function(source, args)
    TriggerServerEvent("ewa-moderatorTag:server:enable")
end)
...
Citizen.CreateThread(function()
    RegisterKeyMapping(Ewa.commandName, 'Moderator Tagg', 'keyboard', Ewa.commandKey)
end)