...
AddEventHandler('esx:playerLoaded', function()
    ESX.TriggerServerCallback('ewa-managementTag:server:requestSync', function(request)
        stafftable = request
    end)
end)

RegisterNetEvent('ewa-managementTagg:client:syncTags')
AddEventHandler('ewa-managementTagg:client:syncTags', function(sync)
    stafftable = sync
end)

RegisterCommand(Ewa.commandName, function(source, args)
    TriggerServerEvent("ewa-managementTag:server:enable")
end)
...
Citizen.CreateThread(function()
    RegisterKeyMapping(Ewa.commandName, 'Management Tagg', 'keyboard', Ewa.commandKey)
end)